...
let
  ...
  themes = {
    ...
    catppuccin-latte = import ../../themes/catppuccin-latte.nix { inherit nix-colors; };
    ...
    everforest = import ../../themes/everforest.nix { inherit nix-colors; };
    nord = import ../../themes/nord.nix { inherit nix-colors; };
    ethereal = import ../../themes/ethereal.nix { inherit nix-colors; };
    flexoki-light = import ../../themes/flexoki-light.nix { inherit nix-colors; };
    hackerman = import ../../themes/hackerman.nix { inherit nix-colors; };
    last-horizon = import ../../themes/last-horizon.nix { inherit nix-colors; };
    lumon = import ../../themes/lumon.nix { inherit nix-colors; };
    matte-black = import ../../themes/matte-black.nix { inherit nix-colors; };
    miasma = import ../../themes/miasma.nix { inherit nix-colors; };
    osaka-jade = import ../../themes/osaka-jade.nix { inherit nix-colors; };
    retro-82 = import ../../themes/retro-82.nix { inherit nix-colors; };
    ristretto = import ../../themes/ristretto.nix { inherit nix-colors; };
    solitude = import ../../themes/solitude.nix { inherit nix-colors; };
    vantablack = import ../../themes/vantablack.nix { inherit nix-colors; };
    white = import ../../themes/white.nix { inherit nix-colors; };
  };
  ...
  # Single centered preview. wallpaper-pick drives navigation:
  #   h/l cycle wallpapers within the current theme (native rofi row-left/right)
  #   j/k cycle themes (custom keybinds; script re-launches rofi per theme)
  # The `message` widget at the top shows the active theme name; only the
  # current theme's wallpapers are passed to listview each iteration.
  ...
  themeSwitchScript = pkgs.writeShellScriptBin "theme-switch" ''
    ...
    # Persist for wallpaper-pick to resume on the active theme
    mkdir -p "$HOME/.cache/rice"
    printf '%s\n' "$THEME" > "$HOME/.cache/rice/current-theme"
  ...
  # wallpaper-pick script — two-axis navigation:
  #   h/l: cycle wallpapers within the current theme (native rofi)
  #   j/k: cycle themes (rofi exits with custom code; loop re-launches)
  #   Enter: switch to selected wallpaper (also swaps active theme via wallpaper-switch)
  # Theme order = order of first appearance in wallpapers/config.json.
  wallpaperPickScript = pkgs.writeShellScriptBin "wallpaper-pick" ''
    ...
    # Distinct themes in config.json order (first occurrence wins)
    mapfile -t THEMES < <(${pkgs.jq}/bin/jq -r 'to_entries[] | .value.theme' "$WALLPAPER_CONFIG" | ${pkgs.gawk}/bin/awk '!seen[$0]++')
    n_themes=''${#THEMES[@]}
    [[ $n_themes -eq 0 ]] && { echo "no themes in $WALLPAPER_CONFIG" >&2; exit 1; }

    # Start on the theme whose wallpaper is currently active (if known),
    # otherwise the first theme.
    theme_idx=0
    if [[ -f "$HOME/.cache/rice/current-theme" ]]; then
      current=$(<"$HOME/.cache/rice/current-theme")
      for i in "''${!THEMES[@]}"; do
        [[ "''${THEMES[$i]}" == "$current" ]] && theme_idx=$i && break
      done
    fi
    ...
    while true; do
      ...
      mapfile -t TKEYS < <(${pkgs.jq}/bin/jq -r --arg t "$cur_theme" 'to_entries[] | select(.value.theme == $t) | .key' "$WALLPAPER_CONFIG")
      n_keys=''${#TKEYS[@]}
      [[ $row_idx -ge $n_keys ]] && row_idx=$(( n_keys - 1 ))
      [[ $row_idx -lt 0 ]] && row_idx=0

      INDEX=$(for k in "''${TKEYS[@]}"; do
        printf '%s\0icon\x1f%s/%s.png\n' "$k" "$THUMBS_DIR" "$k"
      done | ${pkgs.rofi}/bin/rofi -dmenu -show-icons -theme "$ROFI_THEME" \
          -mesg "$cur_theme" \
          -p "" \
          -no-custom \
          -format i \
      ...
      case $rc in
        0)
          [[ -n "$INDEX" ]] && ${wallpaperSwitchScript}/bin/wallpaper-switch "''${TKEYS[$INDEX]}"
          exit 0 ;;
        10) # j → next theme
          theme_idx=$(( (theme_idx + 1) % n_themes ))
          row_idx=0 ;;
        11) # k → previous theme
          theme_idx=$(( (theme_idx - 1 + n_themes) % n_themes ))
          row_idx=0 ;;
        *) exit 0 ;;
      esac
    done
...
in
{
  ...
  home.activation.setupThemeSymlinks = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ...
    # Pre-render wallpaper thumbnails (800x500, center-cropped 16:10) for the
    # single-pane rofi carousel. Idempotent: regenerates only when source is newer.
    WALLPAPER_CONFIG=/home/kamdyns/nixos-config/wallpapers/config.json
    THUMBS_DIR=$HOME/.cache/rice/wallpaper-thumbs
    if [[ -f "$WALLPAPER_CONFIG" ]]; then
      ...
      while IFS=$'\t' read -r key src; do
        ...
        if [[ -f "$src" ]] && [[ ! -f "$dst" || "$src" -nt "$dst" ]]; then
          $DRY_RUN_CMD ${pkgs.imagemagick}/bin/magick "$src" -resize 800x500^ -gravity center -extent 800x500 "$dst" 2>/dev/null || true
        fi
