...
ColumnLayout {
    ...
    function handleWheel(y: real, angleDelta: point): void {
        ...
        if (ch?.id === "workspaces" && Config.bar.scrollActions.workspaces) {
            // Workspace scroll - niri doesn't have special workspaces
            const currentWsId = Config.bar.workspaces.perMonitorWorkspaces
                ? (Niri.monitorFor(screen)?.active_workspace_id ?? 1)
                : Niri.activeWsId;
            if (angleDelta.y < 0 || currentWsId > 1)
                Niri.focusWorkspaceRelative(angleDelta.y > 0 ? -1 : 1);
